...
import { annualPositions, hasData, localDate, periodFor, starPoints } from './model';
    ...
    <main className={`map-main map-${scale}`} aria-label={`${period.label}星图`}>
      ...
      <svg ref={camera} className="star-map" viewBox={viewBox} onClick={nearby} aria-label={`${period.days.length} 个每日星位`}>
        <g stroke="#b8c9e2" strokeWidth=".7">
          {connections.map(({ a, b }) => <line key={b.date} x1={a.x} y1={a.y} x2={b.x} y2={b.y} opacity={records[a.date]?.starBrightness && records[b.date]?.starBrightness ? .2 : .055} />)}
        </g>
        {positions.map(p => {
          ...
          const radius = lit ? base * record.starSize : scale === 'week' ? 6 : 3.8;
